# ruff : noqa: I001
"""
Sample code automatically generated on 2024-11-11 10:12:51
...
variables
  ...
  vector a2
min
  0.5*(a1-a2)'*eigenvectors*diag(((oldsum*eigenvectors'*(a1-a2)).*(eigenvectors'*(a1-a2)))/norm2((eigenvectors'*(a1-a2)).*(eigenvectors'*(a1-a2))))*eigenvectors'*(a1-a2)*(sum((eigenvectors'*(a1-a2)).^2)/norm2((eigenvectors'*(a1-a2)).^2)).^p-y'*(a1-a2)+epsilon*sum(a1+a2)
st
  sum(a1) == sum(a2)
  0 <= a1
  0 <= a2
...
class GenoNLP:
    ...
    def fAndG(self, _x):
        ...
        t_2 = t_1**2
        t_3 = t_1 * t_1
        t_4 = (t_0).dot(self.eigenvectors)
        t_5 = t_4**2
        t_6 = self.np.linalg.norm(t_5)
        t_7 = self.np.sum(t_5)
        t_8 = t_7 / t_6
        t_9 = t_8 ** (self.p - 1)
        t_10 = (t_9 * self.oldsum) * self.p
        t_11 = self.np.linalg.norm((t_4 * t_4))
        t_12 = (t_0).dot((self.eigenvectors).dot((t_1 * t_3)))
        t_13 = (self.np.sum(t_2) / self.np.linalg.norm(t_2)) ** self.p
        t_14 = (0.5 * t_13) * self.oldsum
        t_15 = self.np.linalg.norm(t_3)
        t_16 = (self.eigenvectors).dot((t_3 * t_1))
        t_17 = t_8**self.p
        t_18 = t_17 * self.oldsum
        t_19 = ((t_10 / ((t_6**3) * t_11)) * t_7) * (
            t_12 * (self.eigenvectors).dot((t_2 * t_1))
        )
        t_20 = t_11 * t_6
        t_21 = t_12 * (self.eigenvectors).dot(t_1)
        t_22 = (t_18 / t_11) * t_16
        t_23 = (t_18 / (t_11**3)) * (t_12 * t_16)
        t_24 = self.epsilon * self.np.ones(self.a2_rows)
        f_ = (((t_14 * (t_0).dot(t_16)) / t_15) - (self.y).dot(t_0)) + (
            self.epsilon * self.np.sum((a1 + a2))
        )
        g_0 = (
            (
                (
                    (((((t_10 / t_20) * t_21) - t_19) + ((t_14 / t_15) * t_16)) + t_22)
                    + ((((0.5 * t_17) * self.oldsum) / t_11) * t_16)
                )
                - t_23
            )
            - self.y
                    ...
                    (
                        (
                            (
                                ...
                                - (
                                    (
                                        (0.5 * ((2 * t_9) * (self.oldsum * self.p)))
                                        / t_20
                                    )
                                    * t_21
                                )
                            )
                            - (((0.5 * (t_13 * self.oldsum)) / t_15) * t_16)
                        )
                        - t_22
                    )
                    - (((0.5 * t_18) / t_11) * t_16)
