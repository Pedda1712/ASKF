# ruff: noqa: I001
"""
Sample code automatically generated on 2024-11-09 14:13:59
...
variables
  ...
  vector eigenvalues
min
  0.5*alphas'*((eigenvectors*diag(eigenvalues)*eigenvectors').*Ky)*alphas-sum(alphas)+beta*sum(eigenvalues)+gamma*(eigenvaluesOld-eigenvalues)'*F*(eigenvaluesOld-eigenvalues)
st
  alphas >= 0
  alphas <= c
  Y*alphas == vector(0)
...
class GenoNLP:
    def __init__(
        ...
        assert (
            self.Ky_rows == self.eigenvectors_rows == self.Ky_cols == self.alphas_rows
        )
        assert (
