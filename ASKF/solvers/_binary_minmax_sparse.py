# ruff: noqa: I001
"""
Sample code automatically generated on 2024-11-10 15:20:24
...
variables
  vector a
min
  0.5*(a.*y)'*Q*diag((oldsum*(Q'*(a.*y)).^2)/norm2((Q'*(a.*y)).^2))*Q'*(a.*y)*sum(((Q'*(a.*y)).^2).^p).^(1/p)/norm2((Q'*(a.*y)).^2)-sum(a)
st
  a <= C
  a >= 0
  a'*y == 0
...
class GenoNLP:
    ...
    def fAndG(self, _x):
        ...
        t_6 = (t_0).dot(t_5)
        t_7 = (0.5 * (self.np.sum((t_2**self.p)) ** t_3)) * self.oldsum
        t_8 = self.np.linalg.norm(t_2) ** 2
        t_9 = self.np.sum((t_4**self.p))
        t_10 = self.np.linalg.norm(t_4)
        t_11 = t_10**2
        t_12 = t_9**t_3
        t_13 = t_5 * self.y
        f_ = ((t_7 * t_6) / t_8) - self.np.sum(a)
        g_0 = (
            (
                (
                    (
                        (
                            ((((t_9 ** (t_3 - 1)) * self.oldsum) / t_11) * t_6)
                            * ((self.Q).dot(((t_2 ** (self.p - 1)) * t_1)) * self.y)
                        )
                ...
                + (
                    ((t_12 * self.oldsum) / t_11)
                    * ((self.Q).dot(((t_1 * t_1) * t_1)) * self.y)
                )
            )
            + (
                (((0.5 * t_12) * self.oldsum) / t_11)
                * ((self.Q).dot((t_1 * t_2)) * self.y)
            )
        ) - ((((2 * t_12) * self.oldsum) / (t_10**4)) * (t_6 * t_13))
        g_ = g_0
        return f_, g_
...
# binary ASKF solver interface
# treat beta as p parameter
def solve(
    F,
    _K_old,
    beta,
